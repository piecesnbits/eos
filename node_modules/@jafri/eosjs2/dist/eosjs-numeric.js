...
var ripemd_ts_1 = __importDefault(require("ripemd-ts"));
var bs58_1 = __importDefault(require("bs58"));
var ripemd160 = ripemd_ts_1.default.hash;
...
function stringToPublicKey(s, prefix) {
    if (prefix === void 0) { prefix = 'EOS'; }
    if (typeof s !== 'string') {
 ...
 */
function convertLegacyPublicKey(s, prefix) {
    if (prefix === void 0) { prefix = 'EOS'; }
    if (s.substr(0, 3) === prefix) {
 ...
 */
function convertK1ToLegacyPublicKey(publicKey, prefix) {
    if (prefix === void 0) { prefix = 'EOS'; }
    ...
    var bytesWithChecksum = bs58_1.default.decode(nonPrefixPublicKey);
    var bytes = bytesWithChecksum.slice(0, bytesWithChecksum.length - 4);
    var suffixBytes = Buffer.from(ripemd160(bytes)).slice(0, 4);
    var binaryPublicKey = Buffer.from(__spread(bytes, suffixBytes));
    return "" + prefix + bs58_1.default.encode(binaryPublicKey);
}
exports.convertK1ToLegacyPublicKey = convertK1ToLegacyPublicKey;
/** If a key is in the legacy format (`EOS` prefix), then convert it to the new format (`PUB_K1_`).
 * Leaves other formats untouched
 */
function convertLegacyPublicKeys(keys, prefix) {
    if (prefix === void 0) { prefix = 'EOS'; }
    return keys.map(function (key) { return convertLegacyPublicKey(key, prefix); });
}
